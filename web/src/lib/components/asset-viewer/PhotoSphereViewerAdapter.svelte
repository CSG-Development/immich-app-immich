<script lang="ts">
  ...
  $effect(() => {
    ...
    const onPanoramaError = (event: events.PanoramaErrorEvent) => {
      console.error('Failed to load panorama', event.error);
      onError?.(event.error);
    };
    ...
    viewer.addEventListener(events.PanoramaErrorEvent.type, onPanoramaError);
    ...
    return () => {
      ...
      viewer?.removeEventListener(events.PanoramaErrorEvent.type, onPanoramaError);
    };
  });
