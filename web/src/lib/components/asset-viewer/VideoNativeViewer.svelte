<script lang="ts">
  ...
  const waitForHlsApi = async (el: HlsVideoElement, attempts = 40) => {
    for (let i = 0; i < attempts; i++) {
      if (el.api) {
        return el.api;
      }
      // hls-video-element creates `api` after an internal microtask in load()
      await Promise.resolve();
    }
    return el.api;
  };

  const wireHlsListeners = async (el: HlsVideoElement, assetId: string, resumeTime?: number) => {
    const api = await waitForHlsApi(el);
    if (!api) {
      console.warn('HLS api unavailable, skipping listener wiring');
      return;
    }
    ...
    // MSE/hls.js often skips HTMLMediaElement `progress` events; media-chrome only
    // refreshes the buffered bar on that event, so re-emit when fragments land.
    const notifyBuffered = () => {
      el.dispatchEvent(new Event('progress'));
      // media-chrome may not be subscribed yet on first gallery open — nudge next frame
      requestAnimationFrame(() => el.dispatchEvent(new Event('progress')));
    };

    const prepareLevelsAndStart = async (startTime?: number) => {
      ...
      if (keep.size > 0 && keep.size < api.levels.length) {
      ...
      api.startLoad(startTime);
      notifyBuffered();
    };

    // eslint-disable-next-line @typescript-eslint/no-misused-promises
    api.on(Hls.Events.MANIFEST_PARSED, async () => prepareLevelsAndStart(resumeTime));

    // Client navigation from gallery can wire listeners after MANIFEST_PARSED already fired.
    if (api.levels.length > 0) {
      await prepareLevelsAndStart(resumeTime);
    }

    api.on(Hls.Events.BUFFER_APPENDED, notifyBuffered);
    api.on(Hls.Events.FRAG_BUFFERED, notifyBuffered);
    api.on(Hls.Events.FRAG_LOADED, () => {
      rebuildCount = 0;
      notifyBuffered();
    });
    ...
    api.on(Hls.Events.ERROR, (_, data) => {
      ...
      void wireHlsListeners(el, assetId, resumeTime);
    });
  };
  ...
  $effect(() => {
    ...
    if (videoPlayer && assetFileUrl) {
      ...
      if (isHlsElement(videoPlayer)) {
        ...
        void wireHlsListeners(el, assetId);
      } else {
        videoPlayer.load();
      }
    }
  ...
  const onSeeked = (event: Event) => event.currentTarget?.dispatchEvent(new Event('progress'));
</script>
      ...
      >
        ...
        <div class="flex h-32 w-full flex-col justify-end bg-linear-to-b to-black/80 px-4">
          <media-control-bar part="bottom" class="flex h-10 w-full gap-2">
            <media-play-button class="shrink-0 rounded-full p-2 outline-none text-white">
              <Icon slot="play" icon={mdiPlay} color="white" />
              <Icon slot="pause" icon={mdiPause} color="white" />
            </media-play-button>
            <media-time-display showduration class="rounded-lg p-2 outline-none text-white"></media-time-display>
            ...
            >
              ...
              <media-mute-button class="bg-none p-2 outline-none text-white">
                <Icon slot="off" icon={mdiVolumeMute} color="white" />
                <Icon slot="low" icon={mdiVolumeLow} color="white" />
                <Icon slot="medium" icon={mdiVolumeMedium} color="white" />
                <Icon slot="high" icon={mdiVolumeHigh} color="white" />
              </media-mute-button>
            </div>

            {#if extendedControls}
              <media-fullscreen-button class="shrink-0 rounded-full p-2 outline-none text-white">
                <Icon slot="enter" icon={mdiFullscreen} color="white" />
                <Icon slot="exit" icon={mdiFullscreenExit} color="white" />
              </media-fullscreen-button>
              <media-settings-menu-button class="shrink-0 rounded-full p-2 outline-none text-white"
...
<style>
  media-controller {
    ...
    /* Controls sit on a dark scrim — always white in both app themes */
    --media-focus-box-shadow: 0 0 0 2px white;
    ...
    --media-time-range-buffered-color: rgb(255 255 255 / 0.55);
