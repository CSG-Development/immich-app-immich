<script lang="ts">
  ...
  const handlePasswordSubmit = async () => {
    try {
      sharedLink = await getMySharedLink({ password: value, key, slug });
      setSharedLink(sharedLink);
      passwordRequired = false;
      title = (sharedLink.album ? sharedLink.album.albumName : $t('public_share')) + ' - Personal Cloud Photos';
      description =
    ...
    <div class="flex flex-col items-center justify-center mt-20">
      ...
      <div class="mt-4">
        <form class="flex gap-x-2" novalidate {onsubmit}>
          <PasswordInput autocomplete="off" bind:value placeholder="Password" />
