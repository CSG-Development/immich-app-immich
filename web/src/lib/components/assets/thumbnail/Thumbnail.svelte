  ...
  >
    ...
    >
      ...
      <div class="absolute inset-0 z-2">
        ...
        {#if !authManager.isSharedLink && asset.isFavorite}
          <div class="absolute start-2 bottom-2 z-2">
            <Icon data-icon-favorite icon={mdiHeart} size="24" class="text-white" />
          </div>
        {/if}

        {#if !!assetOwner}
          <div class="absolute end-2 bottom-1 z-2 max-w-[50%]">
        ...
        {#if !authManager.isSharedLink && showArchiveIcon && asset.visibility === AssetVisibility.Archive}
          <div class={['absolute start-2 z-2', asset.isFavorite ? 'bottom-10' : 'bottom-2']}>
        ...
        {#if asset.isImage && asset.projectionType === ProjectionType.EQUIRECTANGULAR}
          <div class="absolute end-0 top-0 z-2 flex place-items-center gap-1 text-xs font-medium text-white">
        ...
        {#if asset.stack && showStackedIcon}
          <div
            class={[
              ...
              asset.isImage && !asset.livePhotoVideoId ? 'end-0 top-0' : 'end-1 top-7',
            ]}
          >
    ...
    {#if mouseOver && onPreview}
      <button
        ...
        class="absolute end-1 bottom-1 z-2 rounded-full bg-black/25 p-1.5 transition-colors hover:bg-black/50 focus:outline-none"
        in:fade={{ duration: 100 }}
        tabindex={-1}
