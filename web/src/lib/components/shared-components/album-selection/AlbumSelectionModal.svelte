<script lang="ts">
  ...
  import AlbumListItem from '../../asset-viewer/AlbumListItem.svelte';
  import NewAlbumListItem from './NewAlbumListItem.svelte';
  ...
  onMount(async () => {
    albums = await getAllAlbums(shared ? { isShared: true } : { isOwned: true });
