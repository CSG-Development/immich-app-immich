...
class MediaCapabilitiesManager {
  ...
  async efficientLevels(levels: Level[]) {
    ...
    // Prefer power-efficient variants; if the browser reports none (common in WSL/VMs),
    // fall back to all supported levels so playback is not left with an empty playlist.
    const preferEfficient = decodingInfo.some((info) => info.powerEfficient);
    // eslint-disable-next-line svelte/prefer-svelte-reactivity
    const lowestBitrateByHeight = new Map<number, number>();
    for (let i = 0; i < levels.length; i++) {
      if (preferEfficient && !decodingInfo[i].powerEfficient) {
        continue;
      }
      if (!decodingInfo[i].supported) {
        continue;
      }
    ...
    if (keep.size === 0) {
      return new Set(levels.map((_, i) => i));
    }
    return keep;
  }
