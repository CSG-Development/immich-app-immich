...
type ZoomOptions = { zoomTarget?: HTMLElement };

export const zoomImageAction = (node: HTMLElement, options?: ZoomOptions) => {
  ...
  const setupOverlayInterception = () => {
  ...
  const ensureZoom = (opts?: ZoomOptions) => {
    if (zoomInstance || !node?.isConnected || !opts?.zoomTarget) {
      return;
    }
    ...
    opts.zoomTarget.style.willChange = 'transform';
    node.style.overflow = 'visible';
    node.style.touchAction = 'none';
  };

  setupOverlayInterception();
  ensureZoom(options);
  ...
  return {
    ...
    destroy() {
      controller.abort();
      if (currentOptions?.zoomTarget) {
        currentOptions.zoomTarget.style.willChange = '';
      }
