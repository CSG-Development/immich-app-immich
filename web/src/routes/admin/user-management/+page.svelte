<script lang="ts">
  ...
  const handleCreate = async () => {
    modalManager.open(UserCreateModal);
    await refresh();
  };

  const handleEdit = async (dto: UserAdminResponseDto) => {
    const result = modalManager.open(UserEditModal, { user: dto, canResetPassword: dto.id !== $user.id });
    switch (result?.action) {
      case 'resetPassword': {
        modalManager.open(PasswordResetSuccessModal, { newPassword: result.data });
        break;
      }
      case 'update': {
        await refresh();
  ...
  const handleDelete = async (user: UserAdminResponseDto) => {
    const result = modalManager.open(UserDeleteConfirmModal, { user });
    if (result) {
      await refresh();
    }
  };
  ...
  const handleRestore = async (user: UserAdminResponseDto) => {
    const result = modalManager.open(UserRestoreConfirmModal, { user });
    if (result) {
      await refresh();
    }
  };
