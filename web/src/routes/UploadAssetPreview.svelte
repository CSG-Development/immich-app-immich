  ...
  <div class="flex items-center gap-2">
    <div class="flex items-center justify-center">
      ...
      {:else if uploadAsset.state === UploadState.STARTED}
        <Icon icon={mdiLoading} size="24" spin class="text-primary" title={uploadAsset.message || $t('asset_uploading')} />
      {:else if uploadAsset.state === UploadState.ERROR || uploadAsset.state === UploadState.UNSUPPORTED_TYPE}
        <Icon icon={mdiAlertCircle} size="24" class="text-danger" title={String(uploadAsset.error ?? $t('error'))} />
