<script lang="ts">
  ...
  const resolvedLocale = $derived($locale === 'default' ? undefined : $locale);

  const visibleMetadataItems = $derived(
    getAllMetadataItems(asset, $t, resolvedLocale)
...
<div class="min-w-60 flex-1 rounded-lg border transition-colors">
  <div class="relative w-full">
    ...
    >
      ...
      <!-- FAVORITE ICON -->
      {#if asset.isFavorite}
        <div class="absolute start-2 bottom-2">
      ...
      <!-- OVERLAY CHIP -->
      <div
        class="absolute end-3 bottom-1 rounded-xl px-4 py-1 text-xs transition-colors {isSelected
          ? 'bg-green-400/90'
    ...
    <button
      ...
      class="absolute start-1 top-1 rounded-full bg-black/35 p-2 text-gray-200 hover:bg-black/50 hover:text-white"
      title={$t('view')}
    >
