  ...
  <section class="flex place-content-center sm:mx-4">
    <Container center size="large" class="pb-28">
      {#if workflows.length === 0}
        <EmptyPlaceholder
          text={$t('create_first_workflow')}
          descriptionText={$t('workflows_help_text')}
      ...
      {:else}
        <div class="my-6 flex flex-col gap-3">
          {#each workflows as workflow (workflow.id)}
            ...
            <Card class="group bg-light shadow-none transition-colors hover:border-primary">
              <CardHeader>
                <div class="flex items-center gap-4" class:opacity-55={!workflow.enabled}>
                  <a href={Route.viewWorkflow({ id: workflow.id })} class="flex min-w-0 flex-1 items-center gap-4">
                    <div
                  ...
                  <div class="shrink-0">
                    <ContextMenuButton
                      ...
                      items={[
                        ...
                        Edit,
                        Duplicate,
                        getWorkflowShowSchemaAction($t, expandedIds.has(workflow.id), () =>
