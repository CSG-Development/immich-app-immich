<script lang="ts">
  ...
  let { stats, isDismissible, isUploading, remainingUploads, removeItem } = uploadAssetsStore;
  ...
  let isDisabled = $derived($uploadAssetsStore.some((asset) => asset.message === $t('asset_hashing')));
...
{#if $isUploading}
  <div
    ...
    class="fixed end-16 bottom-6 z-60"
  >
    {#if showDetail}
      <div
    ...
    {:else}
      <div class="rounded-full">
        <button
          ...
          class="absolute -start-4 -top-4 flex size-10 place-content-center place-items-center rounded-full bg-primary p-5 text-xs text-light"
        >
        ...
        {#if $stats.errors > 0}
          <button
            ...
            in:scale={{ duration: 250, easing: quartInOut }}
            onclick={() => (showDetail = true)}
            class="absolute -end-4 -top-4 flex size-10 place-content-center place-items-center rounded-full bg-danger p-5 text-xs text-light"
          >
