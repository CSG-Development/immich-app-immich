...
class SegmentationRemovalOverlay extends StatefulWidget {
  const SegmentationRemovalOverlay({
    super.key,
    ...
    required this.detectionErrorText,
  });
  ...
  final String logTag;
  final String detectionErrorText;

  @override
  State<SegmentationRemovalOverlay> createState() => _SegmentationRemovalOverlayState();
}

class _SegmentationRemovalOverlayState extends State<SegmentationRemovalOverlay> {
  ...
  Logger get _log => Logger('SegmentationRemovalOverlay.${widget.logTag}');
  ...
  Future<void> _detectSegmentation() async {
    _log.info('[${widget.logTag}] Starting getSegmentationMask()');
    img.Image? mask;
    try {
      mask = await widget.backgroundRemovalService.getSegmentationMask(widget.imageBytes);
    } catch (e, st) {
      _log.severe('[${widget.logTag}] getSegmentationMask threw', e, st);
    }
    if (!mounted) return;
    setState(() {
      _loading = false;
      _initialMask = mask;
      _error = mask == null ? widget.detectionErrorText : null;
    });
  }
  ...
  Future<void> _apply() async {
    ...
    final strokesPayload = _strokeHistory.strokes
        .map((s) => <String, Object>{'x': s.x, 'y': s.y, 'radius': s.radius, 'isAdd': s.isAdd})
        .toList();
    ...
    final maskData = await ImageWorker.instance.buildStrokeMask(
      ...
      baseMask: <String, Object>{'width': base.width, 'height': base.height, 'data': baseData},
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: MaskEditorAppBar(
        ...
        canUndo: _canUndo,
        canRedo: _canRedo,
        onUndo: () => setState(() => _strokeHistory.undo()),
        onRedo: () => setState(() => _strokeHistory.redo()),
      ),
      body: SafeArea(
        child: Column(
          children: [
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: _brushRadius > _minBrushRadius
                          ? () => setState(() {
                              _brushRadius = (_brushRadius - 8).clamp(_minBrushRadius, _maxBrushRadius);
                            })
                          : null,
                    ),
                    Expanded(
