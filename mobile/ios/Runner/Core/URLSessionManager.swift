...
let CLIENT_CERT_LABEL = "com.seagate.curator.stxphotos.ios.client_identity"
let HEADERS_KEY = "immich.request_headers"
let SERVER_URLS_KEY = "immich.server_urls"
let APP_GROUP = "group.com.seagate.curator.stxphotos.ios.share"
...
class URLSessionManager: NSObject {
  ...
  private override init() {
    ...
    super.init()
    // Pinning roots are read at TLS challenge time; recreating the session here
    // would orphan in-flight tasks and race with Dart completion-handler blocks.
...
class URLSessionManagerDelegate: NSObject, URLSessionTaskDelegate, URLSessionWebSocketDelegate {
  ...
  func handleChallenge(
    ...
    case NSURLAuthenticationMethodServerTrust:
      handleServerTrust(challenge, completion: completionHandler)
  ...
  private func handleServerTrust(
    ...
    if evaluateWithSystemTrust(serverTrust, host: host) {
      completion(.useCredential, URLCredential(trust: serverTrust))
    } else {
      completion(.cancelAuthenticationChallenge, nil)
    }
  ...
  private func evaluateWithSystemTrust(_ serverTrust: SecTrust, host: String) -> Bool {
