...
class EndpointInputState extends ConsumerState<EndpointInput> {
  ...
  Future<void> validateAuxilaryServerUrl() async {
    ...
    final isValid = await ref.read(authServiceProvider).validateAuxilaryServerUrl(url);
  ...
  Widget _buildStatusIcon() {
    if (!widget.enabled) {
      return const Icon(Icons.radio_button_unchecked_rounded, color: Colors.grey);
    }
    if (auxCheckStatus == AuxCheckStatus.loading) {
    ...
    if (auxCheckStatus == AuxCheckStatus.valid) {
      return const Icon(Icons.check_circle_rounded, color: Colors.green);
    }
    if (auxCheckStatus == AuxCheckStatus.error) {
