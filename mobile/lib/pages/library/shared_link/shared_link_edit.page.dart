...
class SharedLinkEditPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    useEffect(() {
      ...
      serverUrl.value = DeviceEndpointUtils.buildSharedLinkBaseUrl(connectedDevicePaths);
    ...
    Widget buildSlugField() {
      return TextField(
        ...
        autofocus: false,
        decoration: InputDecoration(
          labelText: slugController.text.isNotEmpty ? 'shared_link_custom_url_title'.tr() : null,
          labelStyle: const TextStyle(fontWeight: FontWeight.bold),
          border: const OutlineInputBorder(),
          hintText: 'shared_link_custom_url_title'.tr(),
    ...
    Future<void> shareLink(String link, BuildContext buttonContext) async {
      ...
      // Required on iPad/macOS; without it share_plus rejects the call and the sheet never appears.
      await Share.share(link, sharePositionOrigin: sharePositionOrigin);
    }
        ...
        decoration: InputDecoration(
          ...
          suffixIcon: Builder(
            builder: (buttonContext) => IconButton(
              onPressed: () => shareLink(link, buttonContext),
    ...
    Future<void> handleNewLink() async {
      ...
      if (newLink != null) {
        final shareUrl = buildSharedLinkUrl(baseUrl: serverUrl.value, slug: newLink.slug, key: newLink.key);
        if (shareUrl == null) {
          ImmichToast.show(
            context: context,
            gravity: ToastGravity.BOTTOM,
