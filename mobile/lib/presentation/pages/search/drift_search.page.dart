...
class DriftSearchPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    void updateExifCurrentFilterWidget(List<SearchExifFilterPair> exifFilters) {
      ...
      exifCurrentFilterWidget.value = Text(_buildExifFilterSummary(validPairs), style: context.textTheme.labelLarge);
    }
    ...
    showExifPicker() {
      var exifFilters = filter.value.exifFilters;
      ...
      handleClear() {
        exifCurrentFilterWidget.value = null;
        search(filter.value.copyWith(exifFilters: []));
      }

      handleApply() {
        updateExifCurrentFilterWidget(exifFilters);
        search(filter.value.copyWith(exifFilters: exifFilters));
      }
        ...
        child: FractionallySizedBox(
          ...
          child: FilterBottomSheetScaffold(
            ...
            child: Padding(
              ...
              child: ExifPicker(onSelect: handleOnSelect, filter: filter.value.exifFilters),
            ),
          ),
        ),
      );
...
String _buildExifFilterSummary(List<SearchExifFilterPair> pairs) {
  return pairs.where((pair) => pair.isComplete).map((pair) => '${pair.tag}: ${pair.value}').take(2).join(', ');
}
