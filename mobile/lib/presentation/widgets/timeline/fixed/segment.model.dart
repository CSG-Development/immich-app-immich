...
class _AssetTileWidget extends ConsumerWidget {
  ...
  bool _getLockSelectionStatus(WidgetRef ref) {
    ...
    // Use refersToSameAsset instead of == / Set.contains: album-fetched assets
    // often have localId=null while the main timeline shows the merged copy with
    // localId set. Fork RemoteAsset.== includes localId (for delete-from-device
    // viewer refresh), so == would miss those matches.
    return lockSelectionAssets.any((a) => a.refersToSameAsset(asset));
  }
