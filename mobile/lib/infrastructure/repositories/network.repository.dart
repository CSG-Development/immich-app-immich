...
class NetworkRepository {
  ...
  /// requests can be aborted and drained at teardown. Enabled only in secondary
  /// isolates via [enableShutdownTracking].
  ...
  ///
  /// The iOS background / gentle isolates share a native [URLSession] across
  /// isolates; if the isolate is destroyed while a request is in flight, the
  /// request's `cupertino_http` delegate later calls back into the dead isolate
  /// and crashes the process. Tracking lets [shutdown] cancel and drain those
  ...
  static Future<void> init() async {
    ...
    if (_trackInFlight && defaultTargetPlatform == TargetPlatform.iOS) {
  ...
  /// (`engine.destroyContext()`) it can leave the shared session's
  /// `cupertino_http` callback target pointing at a destroyed isolate, so the
  /// foreground may no longer complete requests until a cold restart.
  ///
