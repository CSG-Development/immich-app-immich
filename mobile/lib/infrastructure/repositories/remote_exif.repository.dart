...
class RemoteExifRepository extends DriftDatabaseRepository {
  ...
  Future<void> update(
    ...
    Option<String> city = const .none(),
    Option<String> state = const .none(),
    Option<String> country = const .none(),
  }) async {
    if ([dateTimeOriginal, timeZone, location, city, state, country].every((option) => option.isNone)) {
      return;
    }
    ...
    final companion = RemoteExifEntityCompanion(
      ...
      city: city.toDriftValue(),
      state: state.toDriftValue(),
      country: country.toDriftValue(),
    );
