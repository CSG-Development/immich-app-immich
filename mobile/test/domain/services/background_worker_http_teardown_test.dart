...
void main() {
  ...
  test('background isolate enables in-flight HTTP tracking before init', () {
    ...
    final ensureInitAt = source.indexOf('HttpCertPinningManager.ensureInitialized()');
    expect(enableAt, greaterThanOrEqualTo(0));
    expect(
      ensureInitAt,
      greaterThan(enableAt),
      reason: 'tracking must be enabled before HttpCertPinningManager.ensureInitialized',
    );
  });
