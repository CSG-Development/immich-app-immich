...
void main() {
  ...
  group('uploadCandidates', () {
    ...
    late MockAssetEntity mockEntity;

    List<Map<String, String>> stubCandidateUpload() {
      final asset = LocalAssetStub.image1;
      mockEntity = MockAssetEntity();

      when(() => mockBackupRepository.getCandidates(userId)).thenAnswer((_) async => [asset]);
      ...
      when(() => mockStorageRepository.getFileForAsset(asset.id)).thenAnswer((_) async => File('/path/to/image1.jpg'));
      when(() => mockAssetMediaRepository.getOriginalFilename(asset.id)).thenAnswer((_) async => 'image1.jpg');

      return captureFields();
    }

    void stubCapabilities(List<NetworkCapability> capabilities) {
      when(() => mockConnectivityApi.getCapabilities()).thenAnswer((_) async => capabilities);
    }

    test('uploads on unmetered wifi', () async {
      final captured = stubCandidateUpload();
      stubCapabilities([NetworkCapability.wifi, NetworkCapability.unmetered]);
    ...
    test('uploads on metered wifi', () async {
      final captured = stubCandidateUpload();
      stubCapabilities([NetworkCapability.wifi]);
    ...
    test('skips uploads on cellular when cellular backup is disabled', () async {
      final captured = stubCandidateUpload();
      stubCapabilities([NetworkCapability.cellular]);
    ...
    test('uploads on cellular when cellular backup for photos is enabled', () async {
      final settings = SettingsRepository.instance;
      await settings.write(SettingsKey.backupUseCellularForPhotos, true);
      addTearDown(() => settings.write(SettingsKey.backupUseCellularForPhotos, false));
