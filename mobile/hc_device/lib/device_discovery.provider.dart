...
class DeviceDiscoveryController {
    ...
    if (_discovery != null) {
      ...
      _discovery!.addServiceListener((service, status) {
        if (status == nsd.ServiceStatus.found &&
          ...
          _log.info(
            "mDNS Device Found: ${service.toString()}",
          );
          _checkDeviceStatus(
    ...
    try {
      ...
      if (responseList.isSuccessful) {
        ...
        if (remoteDevices != null && remoteDevices.isNotEmpty) {
          _log.info(
            "Found ${remoteDevices.length} remote devices.",
          );
          for (final remoteDevice in remoteDevices) {
            _log.info(
              "Processing remote device: ${remoteDevice.friendlyName}",
            );
            _log.info("${remoteDevice.seagateDeviceID}");
            final responseInfo = await remoteApi.clientV1DevicesDeviceIDGet(
              deviceID: remoteDevice.seagateDeviceID,
            );
            _log.info(
              "Device paths GET for "
      ...
      _log.warning(
        "Remote API error: ${hc_utils.extractErrorMessage(error)}",
      );
      return null;
    }
  }
  ...
  Future<DeviceItem?> _addRemoteDevice(
    ...
    try {
      ...
      _log.warning(
        "Error adding remote device: ${hc_utils.extractErrorMessage(error)}",
      );
      return null;
    }
  }
  ...
  Future<DeviceItem?> _checkDeviceStatus({
    ...
    _log.info("checkDeviceStatus: $baseUrl");
    try {
      final api = await _deviceProvider.createApi(baseUrl: baseUrl);
      ...
      _log.warning(
        "statusGet error: ${hc_utils.extractErrorMessage(error)}",
      );
    }
    return null;
  }
  ...
  Future<DeviceItem?> _getDeviceAbout(
    ...
    try {
      ...
      if (response.isSuccessful && response.body != null) {
        ...
        _log.info(
          "Adding device: ${device.name} at $baseUrl",
        );
      ...
      _log.warning(
        "aboutGet error: ${hc_utils.extractErrorMessage(error)}",
      );
    }
    return null;
  }
